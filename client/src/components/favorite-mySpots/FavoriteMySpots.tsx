...
export const FavoriteMySpots = ({pageType}: TFavoriteMySpotsProps) => {
    ...
    useEffect(() => {
        pageType === `${RoutePath.favorites}` ? fetchFavoriteData() : fetchMySpotsData();
    }, []);

    const fetchFavoriteData = async () => !favSpots && await getFavSpots(dispatch);
    const fetchMySpotsData = async () => !routesPass && await getRoutesPass(dispatch);
